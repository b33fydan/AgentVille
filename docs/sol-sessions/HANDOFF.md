...
- Locally committed the product-identity correction in `d1fe16e` (`Clarify AgentVille product identity`) and its focused evidence packet in `dc3d0bc` (`Harden checkpoint truth`). This slice has not been pushed; pushing still requires owner confirmation.
