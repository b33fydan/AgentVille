...
- The publication artifact is committed in `7d440c4` and pushed. Vercel reports its protected feature-branch Preview successful; `main` is fast-forwarded to the same SHA, while `legacy/react-v3@a93455a` preserves rollback.
- Production is not yet accepted: Vercel reused the preview SHA without creating a Production deployment, so the public alias still serves the legacy bundle. The release-record commit will provide a unique production SHA; accept only after the provider reports Production success and browser checks pass at `https://agent-ville-kappa.vercel.app`.
