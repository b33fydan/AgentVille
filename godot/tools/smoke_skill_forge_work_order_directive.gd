...
func _test_plant_seed_drafts_ready_work_order(scene: Node, game_ui) -> void:
	...
	if order_id == "":
		_fail("Plant Seed did not create a Forge-tagged work order.")
		return
	var order: Dictionary = scene.work_orders.get(order_id, {})
	if str(order.get("action", "")) != "plant_seed" or str(order.get("agent_action", "")) != "plant_seed":
		_fail("Plant Seed work order did not keep the plant_seed directive. order=%s" % str(order))
		return
	if str(order.get("skill_name", "")) != "Plant Seed":
		_fail("Plant Seed work order did not keep a readable skill name. order=%s" % str(order))
		return
	if not str(order.get("label", "")).begins_with("Plant Seed: Plant"):
		_fail("Plant Seed work order did not keep a readable crew work label. order=%s" % str(order))
		return
	if str(order.get("agent_name", "")) != "Marigold":
		_fail("Plant Seed work order did not keep the readable harness agent. order=%s" % str(order))
		return
	var target_tile: Vector2i = order.get("target_tile", Vector2i(-1, -1))
	if not scene.call("_can_target_crew_order", "plant_seed", target_tile):
		_fail("Plant Seed work order target was not an open planting tile. order=%s" % str(order))
		return

	var rows: Dictionary = game_ui.get("_work_order_rows")
	if not rows.has(order_id):
		_fail("Plant Seed work order did not appear in the crew order rows. rows=%s" % str(rows.keys()))
		return
	var row: Dictionary = rows.get(order_id, {})
	var preference := row.get("preference", null) as Label
	if preference == null or not preference.visible or str(preference.text) != "Forge":
		_fail("Plant Seed work order row did not show the Forge context chip.")
		return
	if not str(preference.tooltip_text).begins_with("Forge Work Order: Plant Seed"):
		_fail("Plant Seed chip tooltip did not open with the work-order role. tooltip=%s" % str(preference.tooltip_text))
		return
	...
	var field_log_entries: Array = game_ui.get("_field_log_entries")
	if not _entries_contain(field_log_entries, "Forge order drafted: Plant Seed"):
		_fail("Plant Seed draft did not leave a Field Log receipt. entries=%s" % str(field_log_entries))
		return
