...
- The Forge panel now keeps compact `Run Route:`, `Run Ref:`, `Run Context:`, lifecycle-aware preview/result headers, `Stage:`, `Next Step:`, `Run Receipt:`, and blocked-spec `Forge Drift:` lines plus a compact visible `Run Trail:` label for recent lifecycle stages, while `Run Detail:`/chronological `Run History:` tooltips preserve the latest receipt, repair detail, and full recent history.
