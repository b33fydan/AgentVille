...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview route endpoints, preview/result headers, route-aware preview next steps, Run button wiring, compact route/ref/run-context/`Stage:`/next-step/`Run Receipt:` lines, hidden steady Drift cues, compact visible Run Trail labels, and Field Log/event-log receipts.
...
- The Forge panel now keeps compact route, run refs, run context, lifecycle-aware preview/result headers, `Stage:`, next-step, `Run Receipt:`, and blocked-spec Drift lines plus a compact visible `Run Trail:` label for recent lifecycle stages, while chronological `Run History:` tooltips preserve full receipt and repair details.
