...
- If local licensed MEGAVOXPACK files exist in `assets/megavoxpack_local_preview/`, fence, rock, alternate-rock, flower-patch, tall-grass, tree, and alternate-tree props use those GLBs; otherwise they fall back to the built-in cube art. Keep the raw pack files out of git.
- The starter farm includes one alternate tree near the fields and one alternate rock along the left edge so the optional local art direction is visible on first load.
...
- `tools/smoke_local_megavox_prop_swaps.gd` exercises optional MEGAVOX fence/rock/alternate-rock/flower-patch/tall-grass/tree/alternate-tree swaps, including the starter tree and starter rock, when local files exist and procedural fallbacks when they do not.
