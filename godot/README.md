...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview/result headers, Run button wiring, compact run-detail/current-stage/next-step/receipt lines, hidden steady Drift cues, visible run trails, and Field Log/event-log receipts.
...
- `tools/smoke_skill_forge_spec_preview.gd` exercises structured starter-spec preview fields for trigger, context, ordered tools, steps, checks, receipts, and active preview headers.
