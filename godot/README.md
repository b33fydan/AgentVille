...
- `tools/smoke_skill_forge_work_order_directive.gd` exercises Clear Patch turning a Forge `work_order_directive` into a ready crew-order row while Tend Crops stays receipt-only, including visible `Run Context:`/`Stage:`/next-step/`Run Receipt:` labels plus Forge chip route/trace/stage/current-detail/trace-scan separator/next/lesson cues, labeled route notes, and `Order Blocked:` endpoints.
