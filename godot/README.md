...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview route endpoints, preview/result headers, route-aware preview next steps, Run button wiring, compact route/run-detail/current-stage/next-step/receipt lines, hidden steady Drift cues, compact visible run trails, and Field Log/event-log receipts.
...
- `tools/smoke_skill_forge_work_receipts.gd` exercises sent Forge-authored crew work preserving Forge context in badges, receipts, events, lifecycle-aware result headers, visible route/run-detail/current-stage/next-step/receipt lines, visible run trails, and day-summary work recaps.
...
- The Forge panel now keeps compact route, run-detail, lifecycle-aware preview/result headers, current-stage, next-step, receipt, and blocked-spec Drift lines plus a compact visible run trail for recent lifecycle stages, while chronological tooltip history preserves full receipt and repair details.
