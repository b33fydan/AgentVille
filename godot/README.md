...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview/result headers with labeled receipt detail, route-aware preview next steps, Run button wiring, compact `Run Target:`/`Run Trace:`/`Run Route:`/`Run Ref:`/`Run Context:`/`Stage:`/next-step/`Run Receipt:` lines, hidden steady Drift cues in rows and hovers, compact visible Run Trail labels with shared-skill separators, `[current]` endpoint markers, and `Current Run Detail:` tooltips, and Field Log/event-log receipts.
...
- `tools/smoke_skill_forge_spec_preview.gd` exercises structured starter-spec preview fields for trigger, context, ordered tools, steps, labeled preview tooltip checks/receipts, active preview headers, and crew-order route endpoints.
- `tools/smoke_skill_forge_work_order_directive.gd` exercises Clear Patch turning a Forge `work_order_directive` into a ready crew-order row while Tend Crops stays receipt-only, including visible `Run Context:`/`Stage:`/next-step/`Run Receipt:` labels plus Forge chip stage/current-detail separator/next cues, labeled route notes, and `Order Blocked:` endpoints.
- `tools/smoke_skill_forge_work_receipts.gd` exercises sent Forge-authored crew work preserving Forge context in badges, receipts, events, lifecycle-aware result headers, Forge chip lifecycle current-detail separator and receipt cues, visible route/ref/`Run Context:`/`Stage:`/lifecycle `Next Step:`/explanatory `Run Receipt:` lines for queued/waiting orders, visible Run Trail `[current]` endpoint labels, trace hovers with `Run Target:`, `Run Trace:`, `Current Run Detail:` separators, and chronological `Run History:`, plus day-summary work recaps.
...
- Starter previews now show the structured skill contract directly in the panel: trigger, context, tools, steps, success check, receipt, and whether the route ends in a crew order or a Forge-only receipt, with labeled preview tooltips and matching route-aware `Next Step:` copy.
- The Forge panel now keeps compact `Run Target:`, `Run Trace:`, `Run Route:`, `Run Ref:`, `Run Context:`, lifecycle-aware preview/result headers, `Stage:`, `Next Step:`, `Run Receipt:`, and blocked-spec `Forge Drift:` lines plus a compact visible `Run Trail:` label for recent lifecycle stages with shared-skill separators and a `[current]` endpoint marker, while trace/header/history tooltips preserve labeled `Run Target:`/`Run Trace:`/`Run Context:`/`Run Ref:`/`Run Route:`/`Directive:`/`Tool:`/`Next Step:`/`Run Receipt:` details and `Current Run Detail:` separators/chronological `Run History:` preserve the latest receipt, repair detail, and full recent history.
- Skill Forge runs now land in day summaries as compact Forge recaps, including blocked-run Drift notes.
- Clear Patch now bridges the run harness into the crew-order board: a valid Forge `work_order_directive` drafts a ready `clear_brush` row with a `Forge` chip whose tooltip carries run/order identity, `Run Context:`, directive, tool call, route, stage, current-detail separator, and next-step cues, while Tend Crops remains receipt-only behind a labeled route note until crop-tending work orders exist.
- Sent Forge-authored crew work now keeps Forge context through the active reason badge, queued/waiting Forge chip receipt cues, queued Field Log receipt, agent action event, completed work receipt, and day-summary `forge work` recap without treating Forge as social memory.
