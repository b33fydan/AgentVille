...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview/result headers with labeled receipt and lesson detail, route-aware preview next steps, Run button wiring, compact `Run Target:`/`Run Trace:`/`Run Route:`/`Run Ref:`/`Run Context:`/`Stage:`/next-step/`Run Receipt:` lines, hidden steady Drift cues in rows and hovers, compact visible Run Trail labels with shared-skill separators, `[current]` endpoint markers, and `Current Run Detail:`/`Lesson:` tooltips, and Field Log/event-log receipts.
