...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview route endpoints, preview/result headers, route-aware preview next steps, Run button wiring, compact `Run Route:`/`Run Ref:`/`Run Context:`/`Stage:`/next-step/`Run Receipt:` lines, hidden steady Drift cues, compact visible Run Trail labels with `[current]` endpoint markers and `Current Run Detail:` tooltips, and Field Log/event-log receipts.
...
- The Forge panel now keeps compact `Run Route:`, `Run Ref:`, `Run Context:`, lifecycle-aware preview/result headers, `Stage:`, `Next Step:`, `Run Receipt:`, and blocked-spec `Forge Drift:` lines plus a compact visible `Run Trail:` label for recent lifecycle stages with a `[current]` endpoint marker, while trace/header/history tooltips preserve labeled `Run Context:`/`Run Ref:`/`Run Route:`/`Directive:`/`Tool:`/`Next Step:`/`Run Receipt:` details and `Current Run Detail:`/chronological `Run History:` preserve the latest receipt, repair detail, and full recent history.
