...
- Clear Patch now bridges the run harness into the crew-order board: a valid Forge `work_order_directive` drafts a ready `clear_brush` row with a `Forge` chip whose tooltip carries run/order identity, `Run Context:`, directive, tool call, route, stage, current-detail separator, and next-step cues, while Tend Crops remains receipt-only behind a labeled `Run Route Note:` until crop-tending work orders exist.
