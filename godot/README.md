...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview route endpoints, preview/result headers, route-aware preview next steps, Run button wiring, compact run-detail/current-stage/next-step/receipt lines, hidden steady Drift cues, visible run trails, and Field Log/event-log receipts.
...
- The Forge panel now keeps compact run-detail, lifecycle-aware preview/result headers, current-stage, next-step, receipt, and blocked-spec Drift lines plus a visible run trail for recent lifecycle stages, while tooltip history preserves full receipt and repair details.
