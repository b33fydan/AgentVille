...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview route endpoints, preview/result headers, route-aware preview next steps, Run button wiring, compact `Run Route:`/`Run Ref:`/`Run Context:`/`Stage:`/next-step/`Run Receipt:` lines, hidden steady Drift cues, compact visible Run Trail labels with `Run Detail:` tooltips, and Field Log/event-log receipts.
