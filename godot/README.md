...
- Sent Forge-authored crew work now keeps Forge context through the active reason badge, queued/waiting Forge chip receipt cues, queued Field Log receipt, agent action event, completed work receipt, lifecycle `Lesson` outcome copy, and day-summary `forge work` recap without treating Forge as social memory.
