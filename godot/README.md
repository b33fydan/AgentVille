...
- The starter farm includes one tree near the fields and one alternate rock along the left edge so the optional local art direction is visible on first load.
...
- `tools/smoke_local_megavox_prop_swaps.gd` exercises optional MEGAVOX fence/rock/alternate-rock/flower-patch/tall-grass/tree swaps, including the starter tree and starter rock, when local files exist and procedural fallbacks when they do not.
