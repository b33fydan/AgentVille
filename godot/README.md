...
- `tools/smoke_skill_forge_panel.gd` exercises the first visible Skill Forge panel, template preview selection, preview route endpoints, preview/result headers, Run button wiring, compact run-detail/current-stage/next-step/receipt lines, hidden steady Drift cues, visible run trails, and Field Log/event-log receipts.
...
- Starter previews now show the structured skill contract directly in the panel: trigger, context, tools, steps, success check, receipt, and whether the route ends in a crew order or a Forge-only receipt.
