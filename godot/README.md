...
- The Forge panel now keeps compact route, run refs, run context, lifecycle-aware preview/result headers, current-stage, next-step, receipt, and blocked-spec Drift lines plus a compact visible `Run Trail:` label for recent lifecycle stages, while chronological tooltip history preserves full receipt and repair details.
