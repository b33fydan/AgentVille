...
- `tools/smoke_skill_forge_work_order_directive.gd` exercises Clear Patch, Harvest Crops, and Build Fence turning Forge `work_order_directive` specs into ready crew-order rows while Tend Crops and Plant Seed stay receipt-only, including visible `Run Context:`/`Stage:`/next-step/`Run Receipt:` labels plus Forge chip route/trace/stage/current-detail/trace-scan separator/next/lesson cues, labeled route notes, and target-changed `Order Blocked:` endpoints.
...
- Clear Patch, Harvest Crops, and Build Fence now bridge the run harness into the crew-order board: valid Forge `work_order_directive` specs draft ready `clear_brush`, `harvest_crop`, and `build_fence` rows with labeled `Crew Work Order:` trace detail and a `Forge` chip whose `Forge Work Order:` tooltip carries run/order identity, `Run Context:`, directive, tool call, route, trace path, stage, current-detail separator, trace-scan, next-step, and lesson cues, while Tend Crops and Plant Seed remain receipt-only behind a labeled `Run Route Note:` until those actions have real crew-order paths.
