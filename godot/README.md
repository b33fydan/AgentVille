...
- Starter previews now show the structured skill contract directly in the panel: trigger, context, tools, steps, success check, receipt, and whether the route ends in a crew order or a Forge-only receipt, with labeled `Run Preview:` tooltips and matching route-aware `Next Step:` copy.
...
- Clear Patch now bridges the run harness into the crew-order board: a valid Forge `work_order_directive` drafts a ready `clear_brush` row with labeled `Crew Work Order:` trace detail and a `Forge` chip whose `Forge Work Order:` tooltip carries run/order identity, `Run Context:`, directive, tool call, route, stage, current-detail separator, and next-step cues, while Tend Crops remains receipt-only behind a labeled `Run Route Note:` until crop-tending work orders exist.
