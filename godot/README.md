...
- If local licensed MEGAVOXPACK files exist in `assets/megavoxpack_local_preview/`, fence, rock, and flower-patch props use those GLBs; otherwise they fall back to the built-in cube art. Keep the raw pack files out of git.
...
- `tools/smoke_local_megavox_prop_swaps.gd` exercises optional MEGAVOX fence/rock/flower-patch swaps when local files exist and procedural fallbacks when they do not.
